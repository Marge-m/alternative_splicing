...
def changes(fpkmlist, f2write, genes_fpkm):  
    # print out isoforms with changes, which exceed 0.5 (change in isoform FPKM to gene FPKM ratio)
    ...
    for isoform in filtered.iterrows():
        for gene in genes.iterrows():
            if gene[1]['tracking_id'] == isoform[1]['gene_id']:
                ...
                break
        # check ratio 'isoform FPKM to gene FPKM'
        if gene_fpkm_init != 0 and gene_fpkm_final != 0:
